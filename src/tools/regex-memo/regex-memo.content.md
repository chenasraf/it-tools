...
- Think of a character set as an `OR` operation on the single characters that are enclosed between
  the square brackets.
...
- How word boundary matching works:
  ...
  - Between two adjacent characters within the string, if the first character is `\w` and the second
    character is `\W`.
...
- `\N` is a backreference to the `Nth` capturing group. Capturing groups are numbered starting
  from 1.
