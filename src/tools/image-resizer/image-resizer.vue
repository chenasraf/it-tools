...
function downloadResized() {
  ...
  const ext = outputMimeType.value.split('/')[1] === 'jpeg' ? 'jpg' : outputMimeType.value.split('/')[1];
  ...
  <div flex flex-col gap-3>
    <c-card>
      ...
      <div v-else>
        <div flex flex-col items-center gap-3>
          ...
          >
          <div text-sm op-70>
            {{ sourceFile?.name }} — {{ sourceDimensions?.width }} × {{ sourceDimensions?.height }} px
          </div>
          <c-button @click="clearSource">
    ...
    <c-card v-if="sourceDimensions" title="Resize options">
      ...
      <div v-else-if="mode === 'percentage'">
        <n-form-item label="Percentage (%)">
          <n-input-number v-model:value="percentage" :min="1" :max="1000" placeholder="Percentage" w-full />
        </n-form-item>
      </div>
      ...
      <div mb-3 text-sm op-70>
        Output will be {{ previewTargetDimensions?.width }} × {{ previewTargetDimensions?.height }} px ({{ outputMimeType }})
      </div>

      <div flex justify-center gap-3>
        <c-button :loading="isResizing" @click="applyResize">
          Resize image
        </c-button>
      </div>
    </c-card>
    ...
    <c-card v-if="resizedUrl && resizedDimensions" title="Result">
      <div flex flex-col items-center gap-3>
        ...
        <c-button @click="downloadResized">
          Download
        </c-button>
      </div>
    </c-card>
